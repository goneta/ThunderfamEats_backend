...
<div class="row mt-2 mt-lg-4 row mb-4">
 ...
 <template v-if="!hasData && !is_loading">
	...
	<!-- Debug: show raw data structure if needed
	<pre>{{ datas }}</pre>
	-->
	...
	<div class="row equal align-items-center position-relative">
	   ...
	   <!--
	     Original structure expected datas to be an array of arrays:
	     [[item1, item2, item3]]
	     After backend response normalization, datas[0] contains the list of merchants.
	     We render directly from the first inner array when available.
	   -->

	   <div
	     v-if="datas.length && Array.isArray(datas[0])"
	     class="col-lg-4 mb-3 col-md-6 list-items"
	   ...
	   :class="{ 'make-grey': item && (item.merchant_open_status=='0' || item.close_store=='1' || item.disabled_ordering=='1') }"  >  
	     ...
	     <div class="position-relative"> 	  		   
		   ...
		   <div v-if="item.free_delivery==='1'" class="w-100 d-flex align-items-center justify-content-end" style="position: absolute; bottom: 10px;z-index:9;right:5px;">
			  <el-tag class="ml-2" type="warning"><?php echo t("First Delivery Free")?></el-tag>
		   </div>
	     </div>  
	     <!--END IMAGE-->
