        ...
        <template v-if="MenuStore.data_category[slug]">
          ...
          >
            <div
              ...
              class="text-weight-bold no-margin line-normal q-pl-md"
            >
              ...
              >
                <template v-if="MenuStore.data_items[slug]">
                  <div
                    ...
                    class="light-dimmed absolute-top fit flex flex-center"
                  >
                    <div class="text-dark">{{ $t("Not available") }}</div>
                  </div>
                  <q-item-section
                    avatar
                    v-if="MenuStore.data_items[slug][items_id].has_photo"
                    @click.stop="
                      showItemdetails(
                        category.cat_id,
                  ...
                  >
                    <q-item-label>
                      ...
                      <div
                        class="text-grey ellipsis-2-lines font12 line-normal"
                      >
                        <span
                          v-html="
  ...
  methods: {
    ...
    isItemAvailable(item_id, cat_id) {
      if (Object.keys(this.MenuStore.items_not_available).length > 0) {
        if (this.MenuStore.items_not_available.includes(item_id)) {
          return false;
        }
      }

      if (Object.keys(this.MenuStore.category_not_available).length > 0) {
        if (this.MenuStore.category_not_available.includes(cat_id)) {
          return false;
        }
      }
      return true;
